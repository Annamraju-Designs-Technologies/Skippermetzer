     ...
     for(i=0;i<length;i++){
      ...
      // Check if preview Image exsits
         x = AjaxCall(obj,i);
          if(x==1){
            $(".Gallery").append(
              '<div class="col-md-12 mt-3"><iframe loading="lazy" class="' +
                className +
                '" alt="Image' +
                ImageIndex +
                '" width="100%" height="1000px" class="fadein" id="' +
                ImageIndex +
            ...
            $('.Gallery').append('<div class="col-md-12 mt-3"><embed loading="lazy" class="'+className+'" class="fadein" id="'+ImageIndex+'" onClick="openViewer(this.id)" src="'+newURL+'" type="application/pdf" width="100%" height="100%" /></div>');   
            ImageIndex++;
          }
     }
});
...
function left(){
  $(".ViewerImage").css({"width":"100%","max-height":"100%","top":"0","right":"0","left":"0"});
    // $( ".ViewerImage" ).draggable( { disabled: true });
    $(".ViewerImage").css("object-fit","contain");
    $(".ViewerImage").animate({"height":"100%","max-height":"100%"}, 1);
    $(".ViewerImage").css("height","unset");
    zoomVal=0;
...
if(zoomVal==1){
    ...
    img.onload = function() {
      ...
      // $( ".ViewerImage" ).draggable( { disabled: true });
      if(calc<1.25){
        $(".ViewerImage").animate({"max-height":"100%","top":"0","left":"0"}, 'slow');
        zoomVal=0;
      }else{
        $(".ViewerImage").animate({"width":"100%","max-height":"100%","top":"0","right":"0","left":"0"}, 250);
        $(".ViewerImage").css("height","unset");
